...
function versionFooter(doc) {
  ...
  for (let i = 1; i <= pages; i += 1) {
    ...
    doc.text('Version 2.0 06-Nov-23', pageWidth - MARGIN, pageHeight - 6, { align: 'right' });
  }
}

function drawFormHeader(doc) {
  const pageWidth = doc.internal.pageSize.getWidth();
  doc.addImage(HDFC_LOGO_BASE64, 'PNG', MARGIN, 8, 26, 5.2);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.setTextColor(...VALUE_COLOR);
  doc.text('APPLICATION FOR BANK GUARANTEE', pageWidth / 2, 22, { align: 'center' });
  return 30;
}

function drawBoxedSection(doc, number, title, y, contentFn) {
  // Reserve a generous buffer so a section only just past the fold moves to a fresh page
  // instead of starting here and overflowing mid-section (which would badly miscompute
  // the enclosing border below, since it assumes the whole section stays on one page).
  ...
  doc.text(pdfText(number ? `${number}. ${title}` : title), boxLeft + 2, y + 3.5);

  const contentY = contentFn(y + 8);
  const endPage = doc.internal.getNumberOfPages();

  doc.setDrawColor(...VALUE_COLOR);
  doc.setLineWidth(0.3);
  if (endPage === startPage) {
    doc.rect(boxLeft, startY - 2, boxWidth, contentY - startY + 2);
  } else {
    ...
    doc.rect(boxLeft, startY - 2, boxWidth, pageHeight - 12 - (startY - 2));
    doc.setPage(endPage);
    doc.rect(boxLeft, 10, boxWidth, contentY - 10 + 2);
  }

  return contentY + 4;
}

function drawParagraphs(doc, paragraphs, y, fontSize = 6.5, indent = MARGIN) {
  ...
  paragraphs.forEach((text) => {
    ...
    y = ensureSpace(doc, y, lines.length * 3 + 2);
...
function drawStackedBox(doc, x, y, width, height, label, value) {
  ...
  if (rawValue && rawValue !== '-') {
    ...
    doc.setFontSize(7);
    doc.setTextColor(...VALUE_COLOR);
    const lines = doc.splitTextToSize(rawValue, width - 4).slice(0, Math.floor((height - 3) / 3.2));
    doc.text(lines, x + 2, boxY + 4);
  }
  return boxY + height + 4;
}

// A bordered box with its label to the LEFT — used for short single-line fields.
function drawInlineBox(doc, x, y, width, label, value, { labelWidth = 34, height = 6 } = {}) {
  y = ensureSpace(doc, y, height + 4);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(6.5);
  doc.setTextColor(...LABEL_COLOR);
  const labelLines = doc.splitTextToSize(pdfText(label), labelWidth - 2);
  doc.text(labelLines, x, y + height / 2 + (labelLines.length > 1 ? -1 : 1.2));
  ...
  if (rawValue && rawValue !== '-') {
    ...
    doc.setFontSize(7);
    doc.setTextColor(...VALUE_COLOR);
    doc.text(rawValue, boxX + 2, y + height / 2 + 1.2, { maxWidth: boxWidth - 4 });
  }
  return y + height + 3;
}

function drawSegmentedBoxes(doc, label, value, boxes, y, { x = MARGIN, boxSize = 5 } = {}) {
  ...
  for (let i = 0; i < boxes; i += 1) {
    ...
    if (ch) {
      ...
      doc.setFontSize(7);
      doc.setTextColor(...VALUE_COLOR);
      doc.text(pdfText(ch), bx + boxSize / 2, y + boxSize / 2 + 1.3, { align: 'center' });
    }
    bx += boxSize + gap;
  }
  return y + boxSize + 4;
...
export function downloadBgApplicationPdf(app) {
  ...
  y += 3;
  {
    const branchCodeWidth = c.width;
    const rightY = drawInlineBox(doc, c.right, y + 3, c.width, 'Branch Name', app.branchName, { labelWidth: 26 });
    const leftY = drawSegmentedBoxes(doc, 'Branch Code', app.branchCode, 6, y, { boxSize: 4.5 }) ;
    y = Math.max(leftY, rightY);
  }

  y = drawBoxedSection(doc, 1, 'Type of Application', y, (cy) => {
    cy = drawCheckboxGroup(doc, APPLICATION_TYPES, app.typeOfApplication, cy + 3);
    return drawSegmentedBoxes(doc, 'Amendment (Existing Guarantee Number)', app.amendmentExistingGuaranteeNumber, 15, cy + 2);
  });

  y = drawBoxedSection(doc, 2, 'Applicant Details', y, (cy) => {
    const leftY = drawStackedBox(doc, c.left, cy, c.width, 12, 'Name & Address', app.applicantNameAddress);
    const rightY = drawStackedBox(doc, c.right, cy, c.width, 12, 'Contact Person Name & Mobile No.', app.applicantContactPersonMobile);
    ...
    const r1a = drawInlineBox(doc, c.left, cy, thirdWidth, 'PAN No', app.applicantPan, { labelWidth: 16 });
    const r1b = drawDateBoxes(doc, 'Date of Incorporation', app.applicantDateOfIncorporation, cy, { x: x2, boxSize: 4 });
    const r1c = drawInlineBox(doc, x3, cy, thirdWidth, 'LEI Code', app.applicantLeiCode, { labelWidth: 18 });
    cy = Math.max(r1a, r1b, r1c);

    const leftY2 = drawInlineBox(doc, c.left, cy, c.width, 'Email ID', app.applicantEmail, { labelWidth: 20 });
    const rightY2 = drawStackedBox(doc, c.right, cy, c.width, 10, 'Registered Address', app.applicantRegisteredAddress);
    return Math.max(leftY2, rightY2);
  });

  y = drawBoxedSection(doc, 3, 'Nature of Bank Guarantee', y, (cy) =>
    drawCheckboxGroup(doc, BG_NATURES, app.natureOfBankGuarantee, cy + 3, { columns: 3 })
  );

  y = drawBoxedSection(doc, 4, 'Type of BG', y, (cy) => drawCheckboxGroup(doc, BG_TYPE_OPTIONS, app.typeOfBG, cy + 3));

  y = drawBoxedSection(doc, 5, 'Details of Bank Guarantee', y, (cy) => {
    cy = drawStackedBox(doc, c.left, cy, c.fullWidth, 8, 'Purpose', app.purpose);
    cy = drawInlineBox(doc, c.left, cy, c.fullWidth, 'Amount (Rs/FCY) in figures', app.bgAmountFigures, { labelWidth: 48 });
    cy = drawInlineBox(doc, c.left, cy, c.fullWidth, 'Amount (Rs/FCY) in words', app.bgAmountWords, { labelWidth: 48 });
    ...
    const t1 = drawInlineBox(doc, c.right, tenorLabelY + 2, halfWidth, 'Months', `${app.bgTenorMonths || 0}`, { labelWidth: 20 });
    const t2 = drawInlineBox(doc, c.right + halfWidth + COL_GAP, tenorLabelY + 2, halfWidth, 'Days', `${app.bgTenorDays || 0}`, { labelWidth: 16 });
  ...
  y = drawBoxedSection(doc, 6, 'Beneficiary Details', y, (cy) => {
    const leftY = drawStackedBox(doc, c.left, cy, c.width, 12, 'Name & Address', app.beneficiaryNameAddress);
    const rightY = drawStackedBox(doc, c.right, cy, c.width, 12, 'Contact Person Name & Mobile No', app.beneficiaryContactPersonMobile);
    cy = Math.max(leftY, rightY);

    const leftY2 = drawInlineBox(doc, c.left, cy, c.width, 'EMail ID', app.beneficiaryEmail, { labelWidth: 20 });
    const rightY2 = drawInlineBox(doc, c.right, cy, c.width, 'GST Number', app.beneficiaryGstNumber, { labelWidth: 24 });
    cy = Math.max(leftY2, rightY2);

    const thirdWidth = (c.fullWidth - COL_GAP * 2) / 3;
    const x2 = c.left + thirdWidth + COL_GAP;
    const x3 = x2 + thirdWidth + COL_GAP;
    const r1a = drawInlineBox(doc, c.left, cy, thirdWidth, 'PAN No', app.beneficiaryPan, { labelWidth: 16 });
    const r1b = drawDateBoxes(doc, 'Date of Incorporation', app.beneficiaryDateOfIncorporation, cy, { x: x2, boxSize: 4 });
    const r1c = drawInlineBox(doc, x3, cy, thirdWidth, 'LEI Code', app.beneficiaryLeiCode, { labelWidth: 18 });
    cy = Math.max(r1a, r1b, r1c);

    return drawParagraphs(doc, ['*If e-BG with NeSL, please provide UIN allotted to Beneficiary by NeSL'], cy, 6);
  });

  y = drawBoxedSection(doc, 7, 'Beneficiary Bank Details for SFMS', y, (cy) => {
    const leftY = drawStackedBox(doc, c.left, cy, c.width, 10, 'Name & Address', app.beneficiaryBankNameAddress);
    const rightY = drawInlineBox(doc, c.right, cy, c.width, 'IFSC / SWIFT code', app.beneficiaryBankIfscSwift, { labelWidth: 32 });
    return Math.max(leftY, rightY);
  });

  y = drawBoxedSection(doc, 8, 'Advising Bank Details (if Advising Bank is other than Beneficiary Bank)', y, (cy) => {
    const leftY = drawStackedBox(doc, c.left, cy, c.width, 10, 'Name & Address', app.advisingBankNameAddress);
    const rightY = drawInlineBox(doc, c.right, cy, c.width, 'IFSC / SWIFT code', app.advisingBankIfscSwift, { labelWidth: 32 });
    return Math.max(leftY, rightY);
  });

  y = drawBoxedSection(doc, 9, 'Margin Details', y, (cy) => {
    const halfWidth = (c.width - COL_GAP) / 2;
    let leftY = drawInlineBox(doc, c.left, cy, halfWidth, 'FD No', app.marginFdNo, { labelWidth: 16 });
    const amountY = drawInlineBox(doc, c.left + halfWidth + COL_GAP, cy, halfWidth, 'Amount', pdfCurrency(app.marginFdAmount), { labelWidth: 18 });
    leftY = Math.max(leftY, amountY);
    leftY = drawInlineBox(doc, c.left, leftY, c.width, 'Other', app.marginOther, { labelWidth: 16 });

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6.5);
    doc.setTextColor(...LABEL_COLOR);
    doc.text('New FD', c.right, cy);
    let rightY = drawInlineBox(doc, c.right, cy + 2, c.width, 'Debit A/C', app.marginNewFdDebitAccount, { labelWidth: 20 });
    rightY = drawInlineBox(doc, c.right, rightY, c.width, 'Amount', pdfCurrency(app.marginNewFdAmount), { labelWidth: 18 });

    return Math.max(leftY, rightY);
  });

  y = drawBoxedSection(doc, 10, 'Instruction on Bank Charges', y, (cy) =>
    drawParagraphs(
      doc,
      [
  ...
  y = drawBoxedSection(doc, 11, 'General Declaration', y, (cy) => {
    cy = drawParagraphs(doc, DECLARATION_ITEMS, cy);
    cy = drawParagraphs(
      doc,
      [
    ...
    const leftY = drawInlineBox(doc, c.left, cy, c.width, 'Debit Account Number (for claims)', app.claimDebitAccountNumber, { labelWidth: 48 });
    const rightY = drawInlineBox(doc, c.right, cy, c.width, 'Delayed Payment Interest %', `${app.delayedPaymentInterestPercent || 0}%`, { labelWidth: 48 });
    cy = Math.max(leftY, rightY);
    cy = drawParagraphs(doc, DECLARATION_ITEMS_AFTER_CLAIM, cy);
    cy = drawDateBoxes(doc, 'Date', app.declarationDate, cy + 2, { x: c.left });
    cy = ensureSpace(doc, cy + 2, 10);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.5);
    doc.setTextColor(...LABEL_COLOR);
    doc.text('Authorized Signatory', c.left, cy);
    doc.text('(Stamp to be affixed by firm/ Company)', c.left, cy + 3.5);
    return cy + 5;
  });

  y = drawBoxedSection(doc, null, 'Details of documents enclosed: (please tick all that are relevant)', y, (cy) => {
    cy += 3;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(...VALUE_COLOR);
  ...
  y = drawParagraphs(doc, ['*Please note that in case of e-BG, the stamp duty will be procured through NeSL'], y, 6);
