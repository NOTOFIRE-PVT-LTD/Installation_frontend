...
export function downloadProductionCsv(production) {
  ...
  const rows = (production.lines || []).map((line, index) => ({
    ...
    issuedQty: line.issuedQty ?? Math.max(0, (Number(line.requiredQty) || 0) - (Number(line.pendingQty) || 0)),
  ...
  exportToCsv(
    ...
    [
      ...
      { field: 'issuedQty', headerName: 'Issued Qty' },
      { field: 'pendingQty', headerName: 'Pending Qty' },
